...
\begin{frame}[fragile]{Lifted embedding (types)}{Types can be really bad at error message}
  \vspace{1em}
...
\begin{frame}[fragile]{Direct embedding (macros)}{Macros also can validate and virtualize Scala code}
  \vspace{1em}
...
\begin{frame}[fragile]{Direct embedding (macros)}{Macros can do static checks, but sometimes that non-trivial}
  \vspace{1em}
...
\begin{frame}[fragile]{Shadow embedding (macros + types)}{Based on YinYang, which uses macros and therefore enjoys all their benefits}
  \vspace{1em}
  \begin{exampleblock}{Type signatures are simple and error messages are to the point}
  \begin{semiverbatim}
case class Coffee(id: Int, name: String, ...)
