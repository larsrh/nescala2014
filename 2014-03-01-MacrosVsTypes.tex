...
\documentclass[compress,xcolor={usenames,dvipsnames,table}]{beamer}
...
\fontspec[BoldFont={Fira Sans OT}]{Fira Sans OT Light}
\setmainfont[BoldFont={Fira Sans OT}]{Fira Sans OT Light}
\setmathrm{Fira Sans OT Light}
\setsansfont[BoldFont={Fira Sans OT}]{Fira Sans OT Light}
\setmathsfont(Latin,Greek,Digits){Minion Pro}
\setmonofont[BoldFont={Fira Mono OT}]{Fira Mono OT}
...
\begin{document}
  ...
  \institute{École Polytechnique Fédérale de Lausanne\\Technische Universität München}
  \date{March 1, 2014}
...
\AtBeginSection[]
{
  \begin{frame}
    \vskip40pt
    \begin{center}
      \bfseries\usebeamercolor[fg]{structure}{\Large{\insertsection}}
    \end{center}
  \end{frame}
}
...
\only<4>{With macros you no longer have to choose -- macros can transform pretty solutions into fast code}
...
\visible<2->{def desugared[A](x: A, y: A)(implicit ev: Ring[A]): A =
  \text{\color<2>{blue}{new RingOps(new RingOps(x)(ev).+(y))(ev).*(z) // slow!}}}

\visible<3->{def fast[A](x: A, y: A)(implicit ev: Ring[A]): A =
