...
\begin{frame}[fragile]{Use case: Slick}{An embedded DSL for data access}
  \vspace{1em}
  \begin{alertblock}{Bad: Writing database code in SQL}
    \begin{verbatim}
select c.NAME from COFFEES c where c.ID = 10
  ...
  \vspace{1em}
  \begin{exampleblock}{Good: Writing database code in Scala}<visible@2>
    \begin{verbatim}
...
\begin{frame}{Credits}
  \begin{itemize}
    ...
    \item \text{\color{blue}\href{https://upload.wikimedia.org/wikipedia/commons/7/79/Operation_Upshot-Knothole_-_Badger_001.jpg}{Wikipedia}} for the nuclear explosion picture
